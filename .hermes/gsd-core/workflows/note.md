...
- **Global scope**: `/home/bboyd/src/cos-mcp/.hermes/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/bboyd/src/cos-mcp/.hermes/notes/`)
...
2. Glob `/home/bboyd/src/cos-mcp/.hermes/notes/*.md` (if directory exists) — global notes
...
Global (/home/bboyd/src/cos-mcp/.hermes/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/bboyd/src/cos-mcp/.hermes/notes/` — works in any directory
